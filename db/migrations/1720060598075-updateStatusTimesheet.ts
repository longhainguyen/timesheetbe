...
export class UpdateStatusTimesheet1720060598075 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `ALTER TABLE \`timesheet\` CHANGE \`status\` \`status\` enum ('Pending', 'Approved', 'Rejected', 'NotSubmitted') NOT NULL DEFAULT 'NotSubmitted'`,
        );
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `ALTER TABLE \`timesheet\` CHANGE \`status\` \`status\` enum ('Pending', 'Approved', 'Rejected', 'NotSubmitted') NOT NULL`,
        );
        await queryRunner.query(
            `ALTER TABLE \`timesheet\` ADD CONSTRAINT \`FK_a147ecd5c870c35058f8437b425\` FOREIGN KEY (\`taskId\`) REFERENCES \`task\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
        );
